...
m2.ws <- plm(log_devices_leaving_home ~ trump_vote_share + weeks_since_start,
             data = weeks.since,
...
weeks.since.controls <- weeks.since %>% 
  ...
  ungroup() %>% 
  group_by(county_fips) %>% 
  mutate(lag1 = dplyr::lag(log_devices_leaving_home, n=1L),
         lag2 = dplyr::lag(log_devices_leaving_home, n=2L),
         lag3 = dplyr::lag(log_devices_leaving_home, n=3L),
         lead1 = dplyr::lead(log_devices_leaving_home, n=1L),
         lead2 = dplyr::lead(log_devices_leaving_home, n=2L),
         lead3 = dplyr::lead(log_devices_leaving_home, n=3L)) %>% 
  ungroup()
...
weeks.since.controls.analysis <- weeks.since.controls %>% 
  select(county_fips, weeks_since_start, weeks_since_end, log_devices_leaving_home, trump_vote_share, 
...
```{r}
# MODEL 2: CONTROLS
m2.ws.c <- plm(as.formula(paste("log_devices_leaving_home ~", 
                                paste(n[!n %in% c("log_devices_leaving_home", "county_fips", "weeks_since_end", 
...
m2.ws.ctt <- plm(as.formula(paste("log_devices_leaving_home ~", 
                                paste(n[!n %in% c("log_devices_leaving_home", "county_fips", 
...
```{r}
# MODEL 1: SIMPLE REGRESSION
m2.we <- plm(log_devices_leaving_home ~ trump_vote_share + weeks_since_end,
             data = weeks.since,
...
```{r}
# MODEL 2: CONTROLS
m2.we.c <- plm(as.formula(paste("log_devices_leaving_home ~", 
                                paste(n[!n %in% c("log_devices_leaving_home", "county_fips", "weeks_since_start", 
                                                  "lag1", "lag2", "lag3", "lead1", "lead2", "lead3")], 
                                      collapse = " + "))),
                data = weeks.since.controls.analysis,
                index = c("county_fips", "weeks_since_end"),
...
m2.we.ctt <- plm(as.formula(paste("log_devices_leaving_home ~", 
                                paste(n[!n %in% c("log_devices_leaving_home", "county_fips", 
