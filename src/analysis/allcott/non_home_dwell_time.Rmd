---
title: "Median non-home dwell time"
...
m2.ws <- plm(log_median_non_home_dwell_time ~ trump_vote_share + weeks_since_start,
             data = weeks.since,
             index = c("county_fips", "weeks_since_start"),
             model = "within")

m2.ws.coefs <- tidy(coeftest(
  m2.ws, vcov=vcovHC(m2.ws, type="sss", cluster="group"))) %>% 
...
weeks.since.controls <- weeks.since %>% 
  ...
  ungroup() %>% 
  group_by(county_fips) %>% 
  mutate(lag1 = dplyr::lag(log_median_non_home_dwell_time, n=1L),
         lag2 = dplyr::lag(log_median_non_home_dwell_time, n=2L),
         lag3 = dplyr::lag(log_median_non_home_dwell_time, n=3L),
         lead1 = dplyr::lead(log_median_non_home_dwell_time, n=1L),
         lead2 = dplyr::lead(log_median_non_home_dwell_time, n=2L),
         lead3 = dplyr::lead(log_median_non_home_dwell_time, n=3L)) %>% 
  ungroup()
...
weeks.since.controls.analysis <- weeks.since.controls %>% 
  select(county_fips, weeks_since_start, weeks_since_end, log_median_non_home_dwell_time, trump_vote_share, 
...
```{r}
# MODEL 2: CONTROLS
m2.ws.c <- plm(as.formula(paste("log_median_non_home_dwell_time ~", 
                                paste(n[!n %in% c("log_median_non_home_dwell_time", "county_fips", "weeks_since_end", 
...
m2.ws.c.coefs <- tidy(coeftest(
  m2.ws.c, vcov=vcovHC(m2.ws.c, type="sss", cluster="group"))) %>% 
  tibble::add_row(., term="weeks_since_start-10", 
                  estimate=0, std.error=0, statistic=0, p.value=0, .before=1) %>% 
  mutate(weekorder = as.factor(row_number()),
         percent_difference = -1*estimate,
...
m2.ws.ctt <- plm(as.formula(paste("log_median_non_home_dwell_time ~", 
                                paste(n[!n %in% c("log_median_non_home_dwell_time", "county_fips", 
...
m2.ws.ctt.coefs <- tidy(coeftest(
  m2.ws.ctt, vcov=vcovHC(m2.ws.ctt, type="sss", cluster="group"))) %>% 
...
m2.ws.estimates <- m2.ws.coefs %>% filter(row_number() >= 3 & row_number() <= 15) %>% mutate(model = 1)
m2.ws.c.estimates <- m2.ws.c.coefs %>% filter(row_number() >= 3 & row_number() <= 15) %>% mutate(model = 2)
m2.ws.ctt.estimates <- m2.ws.ctt.coefs %>% filter(row_number() <= 13) %>% mutate(model = 3)
all.ws.estimates <- rbind(m2.ws.estimates, m2.ws.c.estimates, m2.ws.ctt.estimates) %>% 
...
```{r}
# MODEL 1: SIMPLE REGRESSION
m2.we <- plm(log_median_non_home_dwell_time ~ trump_vote_share + weeks_since_end,
             data = weeks.since,
             index = c("county_fips", "weeks_since_end"),
             model = "within")

m2.we.coefs <- tidy(coeftest(
  m2.we, vcov=vcovHC(m2.we, type="sss", cluster="group"))) %>% 
...
```{r}
# MODEL 2: CONTROLS
m2.we.c <- plm(as.formula(paste("log_median_non_home_dwell_time ~", 
                                paste(n[!n %in% c("log_median_non_home_dwell_time", "county_fips", "weeks_since_start", 
                                                  "lag1", "lag2", "lag3", "lead1", "lead2", "lead3")], 
                                      collapse = " + "))),
                data = weeks.since.controls.analysis,
                index = c("county_fips", "weeks_since_end"),
...
m2.we.c.coefs <- tidy(coeftest(
  m2.we.c, vcov=vcovHC(m2.we.c, type="sss", cluster="group"))) %>% 
  tibble::add_row(., term="weeks_since_end-16", 
                  estimate=0, std.error=0, statistic=0, p.value=0, .before=1) %>% 
  mutate(weekorder = as.factor(row_number()),
         percent_difference = -1*estimate,
...
m2.we.ctt <- plm(as.formula(paste("log_median_non_home_dwell_time ~", 
                                paste(n[!n %in% c("log_median_non_home_dwell_time", "county_fips", 
...
m2.we.ctt.coefs <- tidy(coeftest(
  m2.we.ctt, vcov=vcovHC(m2.we.ctt, type="sss", cluster="group"))) %>% 
...
# Combine all 3 models into one plot
m2.we.estimates <- m2.we.coefs %>% 
  filter((row_number() >= 13 & row_number() <= 20)) %>% 
  mutate(model = 1)
m2.we.c.estimates <- m2.we.c.coefs %>% 
  filter((row_number() >= 13 & row_number() <= 20)) %>% 
  mutate(model = 2)
m2.we.ctt.estimates <- m2.we.ctt.coefs %>% 
  filter((row_number() <= 18 & row_number() >= 11)) %>% 
  mutate(model = 3)
all.we.estimates <- rbind(m2.we.estimates, m2.we.c.estimates, m2.we.ctt.estimates) %>% 
  mutate(model = as.factor(model)) %>% 
  group_by(model) %>% 
